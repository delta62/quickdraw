...
import { add as addEvents } from './eventing';
...
/**
 * Given a dom element, assigns a unique id to it if it doesn't have one
 * already. The unique Id is then returned.
 * @param node the element to get the unique id of
...
/**
 * Takes the given dom tree and constructs a virtualized wrapper around it that
 * can be traversed for bindings. This wrapper is used to allow bindings to
 * quickly compute the updates they wish to apply and transparently apply
 * changes to the dom nodes only after all bindings have been evaluated
 */
...
/**
 * @return unwraps a virtual dom node returing the raw node it modifes
 */
export function unwrap(node: VirtualDomNode | HTMLElement | null): HTMLElement | null {
    if (!node) return null;
 ...
 * @param document the document that the node should be imported to
 * @param node a node that should be imported to the given document
 * @param deep whether or not the import should be done recursively
 */
...
export class VirtualDomNode {
    ...
    /**
     * Disposes the entirety of the virtual tree starting from this node
     */
    dispose() {
    ...
    /**
     * @return the raw dom node this virtual node wraps
     */
    getRawNode() {
    ...
    /**
     * Takes all the changes that have been applied to this virtual node and
     * generates a patch object that can be applied by the renderer
     * @return an object describing the patches to apply to the real node
     *     backing this virtual one, or null if there are no patches to apply
     */
    generatePatch() {
     ...
     * @param a virtual dom node to set as this nodes parent
     * @param removeFromOldParent whether or not to remove this node from the old parent
     */
     ...
     * @param templateName The template this node is part of.
     */
     ...
     * @param namespace the namespace to use in storage if not the default
     */
     ...
     * @param namespace (optional) the namespace to use in storage if not the default
     ...
     * @return returns a property of the backing dom node that would normally
     *     be set directly on the node with the dot operator
     */
    ...
    /**
     * Sets a property on the backing dom node that would normally be set
     ...
     * @return returns an attribute set on the dom node
     */
    ...
    /**
     * @param name the name of a css property to read from the node
     * @return the value associated with the given name in the css style object for this node
     */
    ...
    /**
     * @param name the name of a css property to set on the node
    ...
    /**
     * Adds an event listener to the dom node. This method acts as a passthrough to the wrapped node
     ...
     * @param copy if true then a copy of the internal array is returned
     * @return an array of virtual dom nodes that are the children of this node
     */
    ...
    /**
     * @param a real or virtual node to determine if it is a child of this node
     * @return whether or not the given child is a child of this node
     */
     ...
     * @param child the real or virtual dom node to remove from this node
     * @return the virtual child that has been removed
     * @throw if the given child is not a child of this node
     */
     ...
     * @param referenceNode the node to use as a reference point
     */
    insertBefore(child: HTMLElement | VirtualDomNode, referenceNode: HTMLElement | VirtualDomNode) {
        ...
        let childIndex = this._changes.children
            ? this._changes.children.indexOf(virtualChild)
            : -1;
        let referenceIndex = this._changes.children
            ? this._changes.children.indexOf(virtualReference)
            : -1;
     ...
     * @param deep whether or not the node should be recursively cloned
     * @param document the document that the cloned node should belong to
     */
    cloneNode(deep = true, document = this._state.rawNode.ownerDocument) {
        // returns a clone of the virtual and any backing real nodes
        let value;
        let copy = document === this._state.rawNode.ownerDocument
            ? this._state.rawNode.cloneNode(false) as HTMLElement
            : document!.importNode(this._state.rawNode, false) as HTMLElement;
    ...
    private _generateVirtualChildren() {
        ...
        for (let i = 0; i < this._state.rawNode.children.length; i++) {
            let child = this._state.rawNode.children[i] as HTMLElement;
            let virtualChild = virtualize(child);
            virtualChild.setParentNode(this);
            this._changes.children.push(virtualChild);
        }
    ...
    private _generateChildrenActions() {
        ...
        let actions: ChangeSet[] = [ ];
        let prunedChildren = [ ];
        for (let i = 0; i < currentChildren.length; i++) {
            let child = currentChildren[i];
            if (expectedChildren.indexOf(child) === -1) {
                actions.push({ type: "remove", value: child });
            } else {
        ...
        for (let index = 0; index < expectedChildren.length; index++) {
            let value = expectedChildren[index];
            mappingArray[index] = {
        ...
        let actionsForward: ChangeSet[] = actions;
        let arrayForward = prunedChildren;
        let actionsBackward: ChangeSet[] = actions.slice();
        let arrayBackward = prunedChildren.slice();

        // do the forward mapping
        for (let index = 0; index < mappingArray.length; index++) {
            let map = mappingArray[index];
            let curIndex = arrayForward.indexOf(map.value);
        ...
        for (let index = 0, end = mappingArray.length, asc = 0 <= end; asc ? index < end : index > end; asc ? index++ : index--) {
            let map = mappingArray[mappingArray.length - 1 - index];
            let curIndex = arrayBackward.indexOf(map.value);
        ...
        return actionsForward.length > actionsBackward.length
            ? actionsBackward
            : actionsForward;
    }
}
