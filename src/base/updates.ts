...
qdInternal.updates = {
    ...
    updateNodeSet(nodes) {
        ...
        while (i < nodes.length) {
            ...
            let dependency = qdInternal.dom.virtualize(nodes[i++]);

            // get the binding context this node should be evaluated in
            // it is created from the stored data
            let bindingContext = qdInternal.context.get(dependency);
    ...
    schedule(immediately = false) {
        if (qd.getConfig('updatesAsync')) {
            ...
            let updatesState = qdInternal.state.updates;
    ...
    enqueue(domNode) {
        if (!__in__(domNode, qdInternal.state.updates.queue)) {
